//! US1 contract tests (T021–T023): error-path behavior.
//!
//! Covers no-route (T021), unsupported capability (T022), and unidentified
//! protocol (T023). Each error is rendered in the target protocol's native
//! shape when the protocol is known, and the PingoGate-native envelope when it
//! is not — with the HTTP status assigned only at the boundary (constitution IX).
...
fn unknown_model_returns_native_not_found() {
    ...
    assert_eq!(resp.status, 404, "no route maps to 404 (T021)");
...
fn unsupported_capability_returns_native_error() {
    ...
    assert_eq!(
        resp.status, 400,
        "unsupported capability maps to 400 (T022)"
    );
    let json: serde_json::Value = serde_json::from_slice(&resp.body).unwrap();
    assert_eq!(json["error"]["type"], "invalid_request_error");
...
fn unidentified_request_returns_pingogate_native_error() {
    ...
    assert_eq!(resp.status, 400, "unidentified protocol maps to 400 (T023)");
    let json: serde_json::Value = serde_json::from_slice(&resp.body).unwrap();
