...
//!
//! Verifies the two deployment modes the Rust kernel supports
//! (constitution "dual-mode architecture" + spec §12A/§12B):
//!
//! - **standalone**: `PINGO_MODE=standalone` (default) -> `FileSnapshotSource`
//!   builds a `RuntimeSnapshot` from a YAML file + `StaticKeyAuth` authenticates
//!   gateway keys against the snapshot. Exercised end-to-end by reusing the T10
//!   black-box harness: a real subprocess serves a mock upstream, a request
//!   authenticated by the gateway key round-trips. That proves both the
//!   `FileSnapshotSource` and `StaticKeyAuth` code paths fired.
//!
//! - **platform**: `PINGO_MODE=platform` -> `GrpcSnapshotSource` wraps an empty
//!   `SnapshotHolder` and the gRPC server (mTLS + internal token) accepts
//!   snapshot pushes from the Go control plane. The hermetic check below only
//!   verifies the dispatch branch; the full gRPC push contract is covered by
//!   `ctrl-go/internal/snapshot/client_test.go` (Go-side, requires Go running)
//!   and is marked `#[ignore]` here because it needs mTLS certs generated by
//!   `pingogate-ctrl` on first start (R10).
//!
//! - **mode_switch_via_env**: `PINGO_MODE` selects the branch. Hermetic check:
//!   `standalone` + missing config fails with a file-read error; `platform`
//!   without `PINGO_INTERNAL_TOKEN` fails with the platform-required-token
//!   error; an unknown value falls back to standalone. Distinct failure modes
//!   prove the dispatch took different branches without needing the network.
...
fn standalone_mode_uses_file_snapshot_source() {
    ...
    // StaticKeyAuth accepted GATEWAY_KEY and the pipeline routed via the
    // FileSnapshotSource-built snapshot to the mock upstream, which echoes the
    // observed authorization header (the injected upstream key, not the
    // gateway key).
...
fn platform_mode_uses_grpc_snapshot_source() {
    ...
    // Pick a free port for the gRPC listener to avoid collisions.
    let grpc_port = common::free_port();
    let grpc_addr = format!("127.0.0.1:{grpc_port}");

    // 32-byte master key for the AES-GCM KeyVault (constitution XX).
    let mkek = "0123456789abcdef0123456789abcdef";
    ...
    // The gRPC server should come up within a few seconds. If it does, the
    // platform branch ran: GrpcSnapshotSource was constructed and the tonic
    // server bound the port.
    let binds = port_binds_within(grpc_port, Duration::from_secs(10));

    // Capture stderr before killing so the assertion message is informative.
    let _ = child.kill();
    let _ = child.wait();
...
fn mode_switch_via_env() {
    // --- standalone branch: missing config -> file-read error ---
    //
    // We run in a temp dir with no `pingogate-core.yaml` so the default config
    // path resolution fails. The binary should exit non-zero with a Parse
    // error mentioning the config file.
    ...
    assert!(
        ...
        "unset mode should fall back to standalone; got: {stderr_unset}"
    );
