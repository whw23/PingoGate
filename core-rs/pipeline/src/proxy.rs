...
/// `ArcSwap` load per request, authenticates via the injected [`KeyAuth`] impl,
/// and records observability signals via `metrics`.
...
impl GatewayProxy {
    /// Construct with a snapshot holder, a metrics registry, and a key
    /// authenticator (StaticKeyAuth for standalone mode; a platform-mode impl
    /// arrives in S2).
    pub fn new(
        holder: Arc<SnapshotHolder>,
        metrics: Arc<Metrics>,
