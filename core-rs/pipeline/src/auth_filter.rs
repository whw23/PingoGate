...
//! the snapshot (and mapping to a [`Principal`]) is done by the [`KeyAuth`]
//! trait in [`crate::key_auth`]. The gateway key is strictly isolated from
//! upstream provider keys (FR-010).
