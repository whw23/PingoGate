...
//!
//! OpenAI Chat Completions and OpenAI Responses share the same error envelope
//! (rendered by [`pingogate_provider::openai`]); Gemini `generateContent` and
//! Gemini Interactions share the Gemini envelope (rendered by
//! [`pingogate_provider::gemini`]).

use pingogate_core_types::{AppError, ProtocolKind};
use serde_json::json;
...
pub fn render(protocol: Option<ProtocolKind>, err: &AppError) -> (u16, Vec<u8>) {
    let body = match protocol {
        Some(ProtocolKind::OpenAiCompatible) | Some(ProtocolKind::OpenAiResponses) => {
            pingogate_provider::openai::render_error(err)
        }
        Some(ProtocolKind::Anthropic) => pingogate_provider::anthropic::render_error(err),
        Some(ProtocolKind::Gemini) | Some(ProtocolKind::GeminiInteractions) => {
            pingogate_provider::gemini::render_error(err)
        }
        None => json!({
...
mod tests {
    ...
    #[test]
    fn openai_responses_uses_same_shape_as_chat() {
        let (status, bytes) =
            render(Some(ProtocolKind::OpenAiResponses), &AppError::AuthFailed);
        assert_eq!(status, 401);
        let v: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(v["error"]["code"], "invalid_api_key");
    }
    ...
    #[test]
    fn gemini_interactions_uses_same_shape_as_generate_content() {
        let (status, bytes) =
            render(Some(ProtocolKind::GeminiInteractions), &AppError::AuthFailed);
        assert_eq!(status, 401);
        let v: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
