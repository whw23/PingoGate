...
//!
//! - `OpenAiCompatible` (Chat Completions): `usage.prompt_tokens` /
//!   `completion_tokens` with `prompt_tokens_details.cached_tokens` and
//!   `completion_tokens_details.reasoning_tokens`.
//! - `OpenAiResponses` (Responses API): `usage.input_tokens` / `output_tokens`
//!   with `input_tokens_details.cached_tokens` and
//!   `output_tokens_details.reasoning_tokens`. Same field names as Anthropic,
...
/// Responses API: `input_tokens` / `output_tokens` with differently-named
/// details blocks than Chat Completions.
...
mod tests {
    ...
    #[test]
    fn openai_chat_usage_includes_reasoning_and_cache_details() {
    ...
    #[test]
    fn openai_responses_usage_uses_input_output_tokens() {
        let body = br#"{"usage":{"input_tokens":15,"output_tokens":27,"total_tokens":42,
            "input_tokens_details":{"cached_tokens":3},
            "output_tokens_details":{"reasoning_tokens":9}}}"#;
        let usage = parse_usage(ProtocolKind::OpenAiResponses, body).unwrap();
        assert_eq!(usage.input, 15);
        assert_eq!(usage.output, 27);
        assert_eq!(usage.reasoning, Some(9));
    ...
    fn missing_usage_block_yields_none() {
        ...
        assert!(parse_usage(ProtocolKind::OpenAiResponses, br#"{"output":[]}"#).is_none());
    }
